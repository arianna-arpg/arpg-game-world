...
app.whenReady().then(async () => {
  ...
  try {
    ...
    assert.equal(await js("document.querySelectorAll('[data-wd-slot]').length"), 7);
    ...
    assert.equal(await js('__game.account().cosmetics.loadout.slots.playerSkin'), 'moon_glass');
    await js("__game.account().ledger.prologue_lived=1; __game.ui.hideAll(); __game.devStartRun('magician'); __game.ui.showWardrobe(); void 0"); await wait(150);
    ...
    console.log('COSMETICS UI OK: categories, preview, equipment, purchase, reload, Reckoning seal, per-skill overrides, world render, compact layout');
