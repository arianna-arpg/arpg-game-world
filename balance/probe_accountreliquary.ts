...
function home(w: World): void {
  ...
  w.player.pos = { ...anchor.pos }; w.player.invulnerable = true;
}
...
assert.equal(reliquaryCost(savedAccount.reliquary), 100); near(reliquaryPower(savedAccount.reliquary), 0.05);
assert.equal(investReliquary(savedAccount), 100); assert.equal(reliquaryCost(savedAccount.reliquary), 225);
near(reliquaryPower(savedAccount.reliquary), 0.1);
...
w.account.reliquary.rank = 20; w.recalcPlayer(); near(w.player.maxLife() - baseLife, lowValue * 2);
...
assert.equal(w.meta.containers.reliquary.length, 0); assert.equal(w.meta.items.length, 0); near(w.player.maxLife(), baseLife);
...
assert(applySavedCharacter(old, current)); assert.equal(old.meta.items.length, 1, 'reload does not auto-bank modern loose finds');
...
assert.equal(stale.items.length, 0); assert.equal(old.account.reliquary.items[0].affixes[0].rolls[0], 1);
...
mw.account.reliquary.rank = 20; mw.recalcPlayer(); near(follower.sheet.get('damage'), damage * 2);
near(follower.life / follower.maxLife(), 0.4);
mw.player.level = 12; mw.recalcPlayer(); assert.equal(follower.level, 12);
home(mw); mw.oracleRelic(mw.localSeat, idol.uid, 'unseat'); assert(follower.dead);
...
assert(!edge.meta.items.length); assert(!edge.account.reliquary.seated.includes(first.relicKey!));
edge.dropGearAt(edge.player.pos, staleGround, undefined, true);
edge.pickupNearestGear(edge.localSeat); assert(!edge.meta.items.length, 'old ground state cannot duplicate a deposit');
...
near(client.meta.relicEmpowerment!, 0.6); near(client.player.maxLife(), edge.player.maxLife());
