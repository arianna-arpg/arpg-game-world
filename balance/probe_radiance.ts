...
//
// THE HEALED EVIDENCE (the flake-healing recipe, both halves — 2026-07-31).
// The rig's own four mints carry explicit seeds, but a `noBackEdge` mint
// OWNS no road: every exit it ever showed was made by the WORLD around it —
// the eager web + mint-horizon charting resolving its 1-2 rolled '?'
// promises under the web laws (occupancy consolidation, degree caps, the
// roadless-hub fan), neighbours weaving inbound — and that surrounding
// world is diced off the UNSEEDED global die (rollSeed per halo mint,
// AI/weather draws shifting the stream every tick). Measured 1/25 strict
// runs: the dice left probe_v_span_veiled ZERO exits at read time, so the
// permanent-ground check failed `0/0` on its own exits-exist sanity guard —
// sick evidence, healthy law (the wall-clock governor was already pinned in
// bootSimEngine; the die was the last wobble). The heal:
//  · STRUCTURAL — the mints keep the engine's own reachability law (no
//    noBackEdge): the back edge to the gate is the guaranteed exit,
//    UNCONDITIONAL by worldgen's own contract, and a roadless gate hub
//    takes no reciprocal (its rule, not ours) so the fan stays pure.
//  · THE SEEDED SPAN — withSeededRandom swaps Math.random for its own
//    mulberry32 and restores the true die after (THE OFF-STREAM LAW), so
//    the residual web dice are pinned and no other rig shifts.
// Every assertion inside is unchanged.
const RIG_SEED = 0xae5b3d;
withSeededRandom(RIG_SEED, () => {
  ...
  for (let i = 0; i < FAMS.length; i++) {
    ...
    const vz = placeZoneAt({ x: gate.map.x + 90 + i * 70, y: gate.map.y + (i % 2) * 80 }, gate, wa.zoneMap, wa.nextGenId++, {
      ...
      objective: { kind: 'clear' }, seed: (0xbeef ^ (i * 7919)) >>> 0,
