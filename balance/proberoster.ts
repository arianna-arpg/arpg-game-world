...
export const PROBE_ROSTER: readonly ProbeRow[] = [
  ...
  { probe: 'probe_cells.ts', status: 'green', tier: 'fast', why: 'THE CELL FOLD (seamless M1.5, the partition law\'s keel) — determinism, seats inside their own cells, THE NON-OVERLAP LAW over a real grown web, linked pairs meet or leave only a bounded blend wedge, the clamp + the degenerate floor' },
  ...
  { probe: 'probe_seamless.ts', status: 'green', tier: 'fast', why: 'THE RESIDENT RING (seamless M0→M1) — the mode law\'s inertness flag-off, the budgeted ring fill (bands law, one mint per beat, monotone no-flap hysteresis), mints drawn == arrived through recorded partner edges, the tissue-gated open rim with its party leash + THE RIM SEAL consulting the door law itself, THE REBASE AT THE THRESHOLD generalized to any resident (third-zone crossing, world-px continuity, carried floaters, zone-memory replay), demote/reapproach byte-equal re-mints with memory untouched, and the save refusal' },
  { probe: 'probe_seamlesslos.ts', status: 'green', tier: 'fast', why: 'THE CROSS-BORDER SIGHT (seamless M2 wave 7) — castRay routes out-of-arena samples by grid ownership (mouth lines clear both channels, neighbor trunks/wall cells block at their true surfaces, tissue open), the drowsy-lock guards under sight-routed, hold-fire == the swept arrow on the same border geometry, and the mode law\'s byte-identical discrete answers' },
  ...
  { probe: 'probe_tissue.ts', status: 'green', tier: 'fast', why: 'THE CONNECTIVE TISSUE (seamless M0) — sampler purity/determinism over a linked-pair lattice, the sea never walkable, the road ribbon walkable end to end, biome-honest tones off the map wash\'s own tint, the export-only null seam, and the capture law' },
