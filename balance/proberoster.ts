...
export const PROBE_ROSTER: readonly ProbeRow[] = [
  ...
  { probe: 'probe_braintuning.ts', status: 'green', tier: 'fast', why: 'THE BRAINTUNING CENSUS NET — the axis roster parsed from the declaration and held complete both ways against the consumer map, every axis alive in ai.ts AND folded by mergeTuning (a new axis cannot be silently dropped), and the whole authored surface (MONSTERS brains, variants, machine use-layers, ARCHETYPES) swept against the parsed shape — the typo net behind the `as BrainTuning` casts' },
  ...
  { probe: 'probe_skillmodes.ts', status: 'green', tier: 'fast', why: 'THE SKILL-MODE TREES (M0, wild_strike) — the authored fork + whitelist census, the level-5 seal + field discipline, one-pick exclusivity, the resolved view measured through the real cast path (bearings + victims), the persistence round trip with the orphan-drop note, and THE TRANSPARENCY LAW (unpicked ≡ treeless, byte-identical fingerprints)' },
  ...
  { probe: 'probe_summonfill.ts', status: 'green', tier: 'fast', why: 'THE SUMMON FILL — toggled contracts × the sequence grammar: a sequencing gem staggers the ON-fill through the plain path\'s own beat while bare/Legion toggles keep the one-frame muster; the reconciler counts still-emerging bodies as owed (no double-queue, no eviction churn), dismissal purges the trickle, and a dead caster\'s queued emergence dies with them' },
