...
export const CLEAVE_TREE = tree([
    ...
    [sequence(n('heavy_wave', 'Heavy Wave', 'The main thrown axes erupt in blood on impact: enemies within 75 units take 35% of the strike’s damage and Bleed at 15% per second. Catch novas and shed axes cannot trigger this explosion.'),
      ...
      sequence(n('razor_horizon', 'Spreading Wounds', 'After a 0.25-second beat, sweep a broad strip from the opposite side while preparing the axe throw, dealing Cleave’s normal melee damage. Works on its own; with Serrated Wave, this becomes the return sweep of a back-and-forth cut.'),
        { castSweeps: [{ delay: 0.25, delivery: { type: 'melee', range: 65, arcDeg: 140, shape: 'band', fx: 'serratedBackswing' }, power: 1 }] }),
      sequence(n('tempered_wave', 'Serrated Wave', 'On cast, immediately sweep a broad strip in front of you while preparing the axe throw. The sweep deals Cleave’s normal melee damage; the axe follows when the cast completes. With Spreading Wounds, a return sweep follows after a beat.'),
        { castSweeps: [{ delay: 0, delivery: { type: 'melee', range: 65, arcDeg: 140, shape: 'band', fx: 'serratedSweep' }, power: 1 }] })],
