...
export const BIOME_LORE: Record<string, BiomeLore> = {
  ...
  hallowfield:    { title: 'The Hallowfield',   blurb: 'The harvest that no one gathered: oak stands with carved lanterns glowing under the canopies, pumpkin patches ranked to the fence lines. Some of the pumpkins were never pumpkins, and some of the scarecrows turn their heads. The Carven Court keeps these rows now.' },
  ...
  hivesands:      { title: 'The Hivesands',     blurb: 'The chitin country: a scouring dune-hive where the seethe works below the sand. Hive-spires crown the ridges, egg clutches and molt-husk flats between them, and a sky that is usually already moving.' },
  ...
  hag_hollow:     { title: 'The Hag\'s Hollow', blurb: 'The root-cellar under the hovel: peat walls, hung pots, wisplight with no destination it will admit to, and the fen\'s crone at the bottom, who knows your name and dislikes it.' },
  ...
  // THE BLURB PASS (2026-08-08) — the un-lored eighteen: every TILESETS id
  // ships with card copy, so no biome or landmark card reads name-only.
  downs:          { title: 'The Downs',         blurb: 'Rolling barrow heath: tors, hedge-folds and charnel rings standing in open bracken, crypt doors under the mounds, and mummers on the lanes who were never asking for coin.' },
  farmland:       { title: 'Farmland',          blurb: 'The worked shires: hedgerow lanes, crop seas planted in rows deep enough to lose a fight in, and folds the wolves already know. The chattel went feral, the scarecrows kept the schedule, and the harvest court still walks its rounds.' },
  metropolis:     { title: 'The Metropolis',    blurb: 'The walled city: every crossing wears a gate. Boulevards and plazas on one face, tenement warrens on the other, gutter gangs and umbral trades disputing the wards, grates down to the drains and stairs up into the rooms above.' },
  townhouse:      { title: 'The Rooms Above',   blurb: 'Up the stair from the street: lamplit landings, kept rooms and squats, whoever moved in when the tenants moved out. Two flights at most, and the garret ends the climb.' },
  sewerworks:     { title: 'The Sewerworks',    blurb: 'The city\'s underdark: flooded galleries and culverts where the vermin tide runs wall to wall. Wade the seethe, mind the gulpers grown fat below the grates, and remember the drains keep a king.' },
  tableland:      { title: 'The Tablelands',    blurb: 'Red table country standing out of the deep waste: wind-cut mesas, walled sand-courts, and heat-shimmer arguing with every sightline. In the court of sands, one ringed table keeps a mouth that greets you with a question.' },
  courtland:      { title: 'The Courtlands',    blurb: 'The dynasty\'s walled marches on the desert\'s watered rim: a hundred ring-courts, every well behind kilned stone. The open ground is the peril and the walls are the relief; the Sarcophate\'s servants still keep the rounds they were fired for.' },
  river_of_souls: { title: 'River of Souls',    blurb: 'The underworld\'s other artery: a pale meander of soul-mist and slow current, mote-lit bank to bank, docks at every bend and the Pale Ferry working between them. The drowned haul and the shades grieve; the docks never close.' },
  undergrowth:    { title: 'The Undergrowth',   blurb: 'The garden\'s downstairs, chewed rather than built: bores between taproots thick as towers, gall nurseries glowing with brood, root lattices closing the deep ways. Every fight down here happens beside a cradle.' },
  wyrmfields:     { title: 'The Wyrmfields',    blurb: 'The volcanic country\'s deepest face: a plain of quenched melt and slag tors where the ground froze mid-argument. One colossal caldera ring stands anchor on the field, and what sleeps under it keeps a treasury.' },
  kilnhoard:      { title: 'The Kilnhoard',     blurb: 'Under the caldera: a fired gallery where the floor is urns before it is anything else, the Urnfather\'s treasury stacked row on row, and the landlord asleep across the middle of it. Rob it like a surgeon, or meet the mountain.' },
  sleepless_watch:{ title: 'The Sleepless Watch', blurb: 'The crown bowl behind the horn gate: an open-sky watch-court on the pinnacle\'s high terrace, the beacon banked at its heart, the thegns drowsing at their posts. The horn is a clock, and the light comes around.' },
  galestream:     { title: 'The Galestream',    blurb: 'The realm\'s artery: a jetstream of cloud-rafts with somewhere to be. The current has a heading, the gusts shove, and everything that lives here rides the same lane. The Vault of Dawn stands where the stream finally ends.' },
  storm_throat:   { title: 'The Storm-Throat',  blurb: 'The inside of the thunderhead: a charged, close hollow of storm-glass and fulgurite where the Driftways\' weather is made. The tyrant nests at the heart of it, and the calm is not on your side.' },
  wane_vault:     { title: 'The Wane',          blurb: 'The noctarch\'s undercroft below the vesper meadows: moonwells, orreries and comet shards shelved in midnight. The instruments went below when the sky stopped answering; the keeper still winds them.' },
  aether_civitas: { title: 'The Civitas',       blurb: 'The angelic city proper: colossal marble raised on cloud, gleamways and processionals running between pantheon masses you can enter, the Host\'s civic orders on every concourse. The fringe still melts underfoot, and the See crowns the deepest hearts.' },
  aether_cathedral:{ title: 'The Cathedral of the Highest', blurb: 'The city\'s crown: a whole cruciform basilica on the cloud, prayer isles held on spans the sky opens and closes, sunbridges by day and star-spans by night. Past the great west doors the seat of the Highest stands empty, and the Host guards it anyway.' },
  basilica_floor: { title: 'The Basilica Galleries', blurb: 'Up the basilica stair: lit galleries, loggias and choirlofts inside marble the city was hiding, three landings before the belfry ends the climb. Daylight all the way up, and the watchers never blink.' },
};
