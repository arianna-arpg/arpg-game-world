...
import { buyCosmetic, cosmeticAcquisition, cosmeticPick, equipCosmetic, ownsCosmetic, settleCosmetics } from '../meta/cosmetics';
import { drawCosmeticPreview } from '../render/vis/cosmetics';
...
export function renderWardrobe(root: HTMLElement, opts: WardrobeOptions): void {
  ...
  const draw = (): void => {
    ...
    const owned = !!def && ownsCosmetic(account.cosmetics, def.id);
    const preview: CosmeticLoadout = { slots: { ...account.cosmetics.loadout.slots }, skills: {} };
    // Preview the selected skill's resolved pair before applying this category's pending pick.
    if (skill) for (const s of ['skillSkin', 'skillRecolor'] as const) {
      const picked = cosmeticPick(account.cosmetics.loadout, s, skill);
      if (picked) preview.slots[s] = picked.id; else delete preview.slots[s];
    }
    if (selected) preview.slots[slot] = selected; else delete preview.slots[slot];
    root.innerHTML = `<section class="wardrobe" aria-label="Wardrobe">
      ...
      <div class="wd-layout"><aside>
        ...
        <p class="wd-muted">Cosmetics change appearance only. Skills retain their damage, reach and timing.</p>
        ...
        ${candidates.map(d => `<button class="wd-card" data-wd-id="${esc(d.id)}" aria-pressed="${selected === d.id}">
          <span class="wd-state">${effectiveId() === d.id ? 'Equipped' : ownsCosmetic(account.cosmetics, d.id) ? 'Owned' : 'Locked'}</span>
        ...
        <div class="wd-detail"><div class="wd-eyebrow">${def ? esc(def.collection) : 'Your original look'}</div>
          ...
          <div class="wd-actions"><button class="wd-primary" data-wd-equip ${def && !owned ? 'disabled' : ''}>${effectiveId() === selected ? 'Equipped' : 'Equip'}</button>
          ${def?.acquire.kind === 'credits' && !owned ? `<button data-wd-buy ${account.credits < def.acquire.cost ? 'disabled' : ''}>Unlock · ${def.acquire.cost} Mortal Essence</button>` : ''}
    ...
    root.querySelector('[data-wd-equip]')!.addEventListener('click', () => {
      if (equipCosmetic(account, slot, selected, skill || undefined)) { note = `${def?.name ?? 'Original appearance'} equipped. Saved to your account.`; save(); draw(); }
    });
    root.querySelector('[data-wd-buy]')?.addEventListener('click', () => {
      if (selected && buyCosmetic(account, selected)) { note = `${def!.name} unlocked for your account.`; save(); draw(); }
    });
    ...
    const animate = (ms: number): void => {
      ...
      drawCosmeticPreview(canvas, opts.body, preview, ms / 1000);
