...
export interface FeatureHarvest { bonus: [number, number]; always: boolean }

/** A named influence as the zone inherited it. Registry edits may change
 * future discoveries; they must not rename or rebalance existing ground. */
export interface BakedMapFeature {
  id: string;
  kind: string;
  name: string;
  seat: MapCoord;
...
export interface AtlasContext {
  /** Field coordinate sampled at mint, before the graph's display settling. */
  sample: MapCoord;
  seed: number;
  features: BakedMapFeature[];
  /** null deliberately records NO bounty; it is not a request to re-roll. */
  harvest: FeatureHarvest | null;
}

/** Snapshot the SAME hits used by the mint fold, with no second spatial query. */
export function bakeAtlasContext(sample: MapCoord, seed: number, hits: MapFeatureHit[]): AtlasContext {
  return {
    sample: { ...sample }, seed,
    features: hits.map(({ feature: f, def, dist }) => ({
      id: f.id, kind: f.kind, name: f.name, seat: { ...f.seat },
      distance: dist, reach: def.reach, label: def.label,
      icon: def.icon, color: def.color, read: def.read,
    })),
    harvest: foldFeatureHarvest(hits.map(h => h.def.inherit?.harvest)),
  };
}
...
export function foldFeatureHits(hits: MapFeatureHit[]): FeatureFold {
  ...
  for (const h of hits) {
    ...
    if (!inh) continue;
    for (const r of inh.landmarks ?? []) fold.landmarks.push(structuredClone(r));
    for (const r of inh.compositions ?? []) fold.compositions.push(structuredClone(r));
    if (inh.layoutParams) Object.assign(fold.layoutParams, structuredClone(inh.layoutParams));
...
/** Fold FeatureHarvest once at mint; pre-context saves also use this reducer. */
function foldFeatureHarvest(rows: (FeatureInherit['harvest'] | undefined)[]): FeatureHarvest | null {
...
/** Compatibility for pre-context saves, whose feature ids are all we have. */
export function featureHarvestOf(ids: readonly string[] | undefined): FeatureHarvest | null {
  return foldFeatureHarvest((ids ?? []).map(id => featureKindOfId(id)?.inherit?.harvest));
}

export function zoneFeatureHarvest(geo: { features?: string[]; atlas?: AtlasContext } | undefined): FeatureHarvest | null {
  return geo?.atlas ? geo.atlas.harvest : featureHarvestOf(geo?.features);
}
...
// A charted (or scouted) zone reads its BakedMapFeature names. Legacy saves
// with only ids re-name them through the finder's hash.
registerZoneInfoSource((world, zoneId) => {
  ...
  const features = z.geo?.atlas?.features ?? (z.geo?.features ?? []).flatMap(id => {
    const def = featureKindOfId(id);
    return def ? [{ name: featureNameOf(id, seed) ?? def.label,
      label: def.label, icon: def.icon, color: def.color, read: def.read }] : [];
  });
  for (const f of features) {
    rows.push({
      kind: 'condition' as const, icon: f.icon, color: f.color,
      label: f.name.charAt(0).toUpperCase() + f.name.slice(1) + ` (${f.label})`,
      detail: f.read, z: 5,
    });
  }
  return rows;
