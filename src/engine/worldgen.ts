...
import { featuresAt, foldFeatureHits } from '../world/atlas';
...
export function placeZoneAt(
  ...
  const featureFold = spec.fieldBiome && !spec.dimension ? foldFeatureHits(featuresAt(target)) : null;
  const geo = featureFold?.ids.length
    ? { ...(geo0 ?? {}), features: featureFold.ids, ...(featureFold.relief ? { relief: featureFold.relief } : {}) }
    : geo0;
