...
| `aether_cathedral` tileset | `src/data/tilesets.ts` | `biome: aether_civitas` — the TRUE city's crown; `depthAffinity { from: 0.86 }` seats the See at the Civitas' deepest hearts. Contact-only collapse, sun/star span rows, `singing_refrain` puzzles, the clergy's pack table. |
