...
Actor-to-actor resident prompts pass both actors' committed stories through
the optional fourth argument to `dwellReachable`. Different stories refuse
the prompt; equal stories set the ray heights explicitly. Inferring from a
stacked floor cell can otherwise mistake a ground-floor patron for someone
upstairs and make the conversation flicker behind unrelated partitions.
