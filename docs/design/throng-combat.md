...
**Integration dependency:** the companion starting-skill-tree change supplies
`World.promoteCommandThrong`, its commandMinions call, and the buff-aware
demotion guard. Those three hunks were applied locally for verification but
are deliberately excluded from this commit. They make pooled gnats eligible
for the same ordinary command handlers as full actors. Apply the companion
batch before this commit and rerun the combined checks.
